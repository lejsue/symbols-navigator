...
export default class TreeView {
  ...
  onClick(event) {
    ...
    if (target.classList.contains('list-nested-item')) {
      ...
      currentNode = target.closest('.list-nested-item');
      if(atom.config.get('symbols-navigator.collapseClick') === 'Go to symbol'){
      ...
      }else if(atom.config.get('symbols-navigator.clickType') === 'Single Click'){
        collapsed = true;
      }
  ...
  moveToSelectedSymbol() {
    ...
    if (selectedNode.dataset.row && selectedNode.dataset.row >= 0 && editor != null) {
      ...
      // setting the cursor has to be delayed for scrolling to work correctly
      setTimeout(() => {
